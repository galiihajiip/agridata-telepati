...
| 5 | Weights valid | PASS | `runs/detect/final/final_model/weights/best.pt`, 6,250,154 bytes, SHA-256 `a48da07d91188b9985171bda8c2ebc745699fb21a1fa059291b014e6951b12e6` — matches `weights/best.pt.sha256` and `final_model_metadata.json`, recomputed and confirmed identical in this session. |
...
| 2 | Staged commit history | PASS | Same evidence as REPRODUCIBILITY #5 — one commit per block, semantic messages (where not overtaken by the user's own auto-commit daemon; every such occurrence has been disclosed in that block's status report). |
