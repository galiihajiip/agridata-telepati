...
def build_markdown_report(per_split_results: dict[str, dict]) -> str:
    lines = ["# Ringkasan Eksplorasi Dataset", "", "Dihasilkan oleh `scripts/visualize_dataset.py`, yang hanya membaca dataset mentah tanpa mengubahnya.", ""]
    for split, result in per_split_results.items():
        lines += [
            f"## Split: `{split}`",
            "",
            f"- Jumlah citra: {result['num_images']}",
            f"- Jumlah anotasi, hanya kelas canonical: {result['num_annotations']}",
            f"- Citra tanpa anotasi: {result['images_with_zero_annotations']}",
            f"- Anotasi terbanyak dalam satu citra, adegan terpadat: {result['max_annotations_in_one_image']}",
            f"- Kelas paling jarang, instance tersedikit: {result['rarest_classes']}",
            f"- Kelas paling sering muncul: {result['most_common_classes']}",
            f"- Kelas dengan median luas bbox terkecil, berisiko objek kecil: {result['smallest_object_classes']}",
            "",
        ...
        lines.append("")
        lines.append(f"Figur: `{result['saved_figures']}`")
        lines.append(f"Contoh beranotasi yang disimpan: {len(result['saved_samples'])}")
        lines.append("")
    return "\n".join(lines)
