...
def main() -> int:
    ...
    md_lines = [
        ...
        "",
        f"Bobot: `{args.weights}` | Split: `{args.split}` | Ambang confidence: {args.confidence_threshold}",
        "",
        "## Rincian jumlah",
        "",
        f"- True positive: {summary['counts']['true_positives']}",
        f"- Salah kelas (lokasi benar, label salah): {summary['counts']['class_confusions']}",
        f"- False positive terhadap latar (mendeteksi objek yang tidak ada): {summary['counts']['background_false_positives']}",
        f"- False negative (objek terlewat sepenuhnya): {summary['counts']['false_negatives']}",
        "",
    ...
    md_lines += [
        ...
        "",
        f"- Confidence true positive: {conf_stats['true_positive_confidence']}",
        f"- Confidence false positive: {conf_stats['false_positive_confidence']}",
        "",
        "## Analisis objek kecil yang terlewat",
        "",
        f"- Median luas bbox ground truth keseluruhan: {small_object_miss_analysis['overall_median_gt_area_px2']} px²",
        f"- Median luas bbox yang terlewat: {small_object_miss_analysis['false_negative_median_area_px2']} px²",
        f"- Objek terlewat cenderung lebih kecil dari rata-rata: {small_object_miss_analysis['false_negatives_are_smaller_than_average']}",
        "",
        "## Adegan padat dibanding adegan jarang",
        "",
        f"- Rasio false negative pada adegan padat, lebih dari {crowded_threshold} instance per citra: {summary['crowded_vs_sparse_scene_fn_rate']['crowded_scene_fn_rate']}",
        f"- Rasio false negative pada adegan jarang: {summary['crowded_vs_sparse_scene_fn_rate']['sparse_scene_fn_rate']}",
        "",
        ...
        "## Contoh visual",
        "",
        f"- False positive: {fp_examples}",
        f"- False negative: {fn_examples}",
        "",
